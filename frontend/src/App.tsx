...
const newPatient = (): Patient => ({
  id: 'DM-26006', name: 'Sunita Verma', age: 52, sex: 'Female', diabetesDuration: '8 years', lastScreening: 'First screening', contact: '+91 9â€¢â€¢â€¢ â€¢â€¢â€¢ 210', grade: 3, status: 'Priority referral', reviewStatus: 'Not reviewed', screenDate: today,
})
...
function App() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/predict_v3_4`, { method: 'POST', body })
      ...
      if (rawText) {
        try {
          ...
          console.log('[DrishtiMitra API RESPONSE]', result)
        } catch {
  ...
  const completeRegistration = (form: HTMLFormElement) => {
    ...
    const patient: Patient = { ...newPatient(), name, age, sex, diabetesDuration: String(data.get('duration') || '8 years'), contact: String(data.get('contact') || '+91 9â€¢â€¢â€¢ â€¢â€¢â€¢ 210') }
    setSelectedPatient(patient)
...
function ExplainStep({ detail, prediction, onBack, onNext }: { detail: GradeDetails; prediction: ModelPrediction | null; onBack: () => void; onNext: () => void }) {
  ...
  return <><div className="rounded-2xl border border-teal-100 bg-teal-50 px-5 py-4 text-sm leading-6 text-teal-950"><strong>Evidence summary:</strong> {detail.label} {prediction?.heatmap ? 'Â· Live PyTorch Grad-CAM Heatmap Generated' : 'Â· Prototype Visuals'} Â· <strong>{detail.recommendation}</strong></div><div className="mt-6 grid gap-6 xl:grid-cols-2"><HeatmapViewer detail={activeDetail} /><LesionMaskViewer detail={detail} /></div><div className="mt-6 flex justify-between gap-3"><button onClick={onBack} className="btn-secondary">â† Screening result</button><button onClick={onNext} className="btn-primary">Generate research report â†’</button></div></>
}
...
function ReportStep({ patient, detail, online, prediction, showReferral, referralId, onBack, onRefer, onReferralCreated, onReview, onReport }: { patient: Patient; detail: GradeDetails; online: boolean; prediction: ModelPrediction | null; showReferral: boolean; referralId: string | null; onBack: () => void; onRefer: () => void; onReferralCreated: (id: string) => void; onReview: () => void; onReport: () => void }) {
  return <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]"><div><ReportViewer patient={patient} detail={detail} online={online} prediction={prediction} /><div className="mt-5 flex flex-wrap gap-3"><button onClick={() => window.print()} className="btn-primary">ðŸ“„ Generate Patient Report (PDF)</button><button onClick={onReport} className="btn-secondary">Open full report page</button><button onClick={onReview} className="btn-secondary">Open specialist review â†’</button></div></div><aside>{referralId ? <div className="rounded-2xl border border-teal-200 bg-teal-50 p-6 shadow-sm"><span className="grid h-11 w-11 place-items-center rounded-full bg-teal-700 text-lg font-black text-white">âœ“</span><p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-teal-700">Referral Created</p><h3 className="mt-1 text-xl font-black text-slate-950">{referralId}</h3><p className="mt-3 text-sm leading-6 text-slate-700">Referral workflow is queued for specialist review. This is a prototype integration / workflow demonstration.</p><button onClick={onReview} className="btn-primary mt-5 w-full">Open ophthalmologist review</button></div> : showReferral ? <ReferralPanel patient={patient} detail={detail} onCreated={onReferralCreated} /> : <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><p className="eyebrow">Next step</p><h3 className="mt-1 text-xl font-bold text-slate-950">Refer with evidence</h3><p className="mt-4 text-sm leading-6 text-slate-600">{prediction ? 'The research report includes the uploaded image and experimental classifier output. It does not include patient-specific heatmaps or lesion masks.' : 'The demonstration report includes prototype evidence visualizations and a recommendation for specialist consideration.'}</p><button onClick={onRefer} className="btn-primary mt-5 w-full">Refer to specialist</button><div className="mt-5 rounded-xl bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Referral flow</p><p className="mt-2 text-sm leading-6 text-slate-700">Rural Health Centre â†’ DrishtiMitra â†’ Screening Report â†’ e-Sanjeevani / Telemedicine â†’ Ophthalmologist â†’ Final clinical review / sign-off</p></div></div>}<button onClick={onBack} className="btn-secondary mt-5">â† Back to evidence</button></aside></div>
}
